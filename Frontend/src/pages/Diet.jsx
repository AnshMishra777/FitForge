  ...
  return (
    <div className="mb-2.5">
      <div className="flex justify-between mb-1">
        <span className="text-[11px] font-semibold text-forge-muted">{label}</span>
        <span className="text-[11px] font-mono text-forge-text">{value}{unit}</span>
      </div>
  ...
  return (
    <div className={`flex gap-2.5 mb-4 ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
      ...
      <div className={`w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center text-sm ${
        isUser
          ? 'bg-gradient-to-br from-forge-orange to-orange-400 text-white font-bold'
      ...
      <div className={`max-w-[80%] px-4 py-3 rounded-2xl text-sm leading-relaxed ${
        isUser
          ? 'bg-forge-orange/15 border border-forge-orange/25 text-forge-text rounded-tr-sm'
          : 'bg-forge-card border border-forge-border text-forge-muted rounded-tl-sm'
      }`}>
        {msg.loading ? (
...
export default function Diet() {
  ...
  const generateFallback = (query) => {
    ...
    if (q.includes('egg')) {
      return `📊 Nutritional Analysis — Large Eggs (3 eggs, ~150g)\n\n━━ MACRONUTRIENTS ━━\n• Calories: 216 kcal\n• Protein: 18g\n• Carbohydrates: 1.2g\n• Fat: 15g (saturated 5g)\n\n━━ KEY VITAMINS ━━\n• Vitamin D: 12% DV\n• Vitamin B12: 33% DV\n• Vitamin A: 20% DV\n• Choline: 630mg\n\n━━ MINERALS ━━\n• Selenium: 69% DV\n• Phosphorus: 27% DV\n\n━━ FITNESS CONTEXT ━━\nEggs contain all 9 essential amino acids with exceptional bioavailability (~91%). Don't skip the yolk.\n\n⏱ TIMING: Excellent for breakfast or any meal.\n\n💡 TIP: Whole eggs produce 40% greater muscle protein synthesis than egg whites alone.`
    }

    if (q.includes('oat')) {
      return `📊 Nutritional Analysis — Cooked Oatmeal (1 cup / 240g)\n\n━━ MACRONUTRIENTS ━━\n• Calories: 166 kcal\n• Protein: 5.9g\n• Carbohydrates: 28g (fiber 4g, sugar 0.6g)\n• Fat: 3.6g\n\n━━ MINERALS ━━\n• Manganese: 86% DV\n• Magnesium: 16% DV\n• Iron: 12% DV\n\n━━ FITNESS CONTEXT ━━\nOatmeal is the gold standard pre-workout carb. Beta-glucan fiber provides slow, sustained energy.\n\n⏱ TIMING: Best 60–90 minutes before training.\n\n💡 TIP: Add a scoop of protein powder and banana for a complete pre-workout meal.`
    }

    if (q.includes('salmon')) {
      return `📊 Nutritional Analysis — Atlantic Salmon (100g)\n\n━━ MACRONUTRIENTS ━━\n• Calories: 208 kcal\n• Protein: 20g\n• Fat: 13g (Omega-3: 2.5g!)\n• Carbohydrates: 0g\n\n━━ KEY VITAMINS ━━\n• Vitamin D: 127% DV\n• Vitamin B12: 133% DV\n\n━━ FITNESS CONTEXT ━━\nOmega-3s (EPA + DHA) significantly reduce exercise-induced muscle inflammation and soreness.\n\n⏱ TIMING: Ideal for dinner or post-workout meals.\n\n💡 TIP: Aim for 2–3 servings of fatty fish per week.`
    }

    if (q.includes('banana')) {
      return `📊 Nutritional Analysis — Medium Banana (118g)\n\n━━ MACRONUTRIENTS ━━\n• Calories: 105 kcal\n• Carbohydrates: 27g (sugar 14g, fiber 3.1g)\n• Protein: 1.3g\n• Fat: 0.4g\n\n━━ MINERALS ━━\n• Potassium: 422mg ★\n• Magnesium: 8% DV\n\n━━ FITNESS CONTEXT ━━\nBananas are the perfect pre/during-workout fuel. Potassium prevents muscle cramps.\n\n⏱ TIMING: 30 min before training or immediately post-workout.\n\n💡 TIP: Pair with peanut butter for sustained energy.`
    }
  ...
  return (
    <div className="flex h-[calc(100vh-60px)] overflow-hidden bg-forge-bg">

      {/* ══ SIDEBAR ══════════════════════════════════════════════ */}
      <aside className={`
        w-72 border-r border-forge-border bg-forge-surface flex flex-col flex-shrink-0
      ...
      `}>
        ...
        <div className="p-5 border-b border-forge-border">
          <div className="flex items-center gap-3 mb-1">
            ...
            <div>
              <p className="font-display text-lg tracking-[0.06em] text-forge-text">AI Nutrition</p>
              <p className="text-[10px] text-forge-dim font-mono">Powered by Claude AI</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          <div className="flex flex-col gap-1.5">
            {QUICK.map((q, i) => (
              <button
                ...
                className="text-left text-xs text-forge-muted hover:text-forge-text px-3 py-2.5 rounded-lg border border-transparent hover:border-forge-border hover:bg-forge-card transition-all leading-relaxed"
              >
                {q}
              </button>
            ))}
          ...
          <div className="mt-5 pt-5 border-t border-forge-border">
            ...
            <p className="text-[10px] text-forge-dim mt-3">Reference values for active adult (2000 kcal diet)</p>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-forge-border">
          <button
            onClick={clearChat}
            className="w-full py-2 rounded-lg border border-forge-border text-forge-dim text-xs font-bold hover:border-forge-borderLight hover:text-forge-muted transition-colors"
          >
            Clear Conversation
          </button>
        </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        {/* Chat header */}
        <div className="flex items-center justify-between px-5 py-3 border-b border-forge-border bg-forge-surface/80 backdrop-blur-md flex-shrink-0">
          <div className="flex items-center gap-3">
            <button
              onClick={() => setSidebarOpen(true)}
              className="md:hidden text-forge-muted hover:text-forge-text text-lg transition-colors mr-1"
            >☰</button>
            <div>
              <p className="font-semibold text-sm text-forge-text">AI Nutrition Coach</p>
              <p className="text-[10px] text-forge-dim">Ask about any food, meal, or nutrition topic</p>
            </div>
          </div>
        ...
        {/* Input area */}
        <div className="flex-shrink-0 border-t border-forge-border bg-forge-surface p-4">
          <div className="max-w-2xl mx-auto">
            <div className="flex gap-3 items-end">
              <textarea
                ...
                rows={2}
                className="flex-1 px-4 py-3 bg-forge-card border border-forge-border rounded-xl text-sm text-forge-text outline-none resize-none font-body leading-relaxed placeholder:text-forge-dim focus:border-forge-orange transition-colors"
              ...
              <button
                ...
                className={`w-11 h-11 rounded-xl flex items-center justify-center text-lg font-bold transition-all flex-shrink-0 ${
                  input.trim() && !loading
                    ? 'bg-forge-orange text-white shadow-[0_2px_16px_rgba(255,90,31,0.35)] hover:bg-orange-500 active:scale-95'
                    : 'bg-forge-card text-forge-dim cursor-not-allowed border border-forge-border'
                }`}
              >
                {loading
                  ? <span className="w-4 h-4 rounded-full border-2 border-forge-border border-t-forge-orange animate-spin" />
                  : '↑'
                }
              </button>
            </div>
            <p className="text-[10px] text-forge-dim text-center mt-2.5">
